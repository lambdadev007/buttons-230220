...
const StarsRating = () => {
    ...
    return (
    <>
        <nav className={classNames("rating", animateDir==1?"animate-left": animateDir==2?"animate-right":"")} 
            // style={{'--x' : xValue }}
            ref = {ratingRef}
        >
          <ul>
          {
            [0,1,2,3,4].map(i => (
                <li key={i} 
                    ...
                    className={classNames(i===current && flag === false ?"current":"", moveToIndex > i ? "active" : "", flag && moveToIndex === i ?"move-to":"", flag && current === i ? "move-from":"")} 
                    onClick={()=>clickStar(i)}>
                    <span></span>
                    <svg>
                        <use xlinkHref="#star" />
